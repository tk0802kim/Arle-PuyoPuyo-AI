...
for epoch in range(nepoch):
    for i in range(len(memory_lane)):
        ...
        if np.random.rand(1)<eps:
            ...
            viewstate = np.reshape(qf.agent_view(memory_lane[i].cur_gs),(1,461))
            #move forward in Q
        ...
        viewstate_cur_batch = np.zeros(shape=(batch_size,461)) #list of currernt states in agent viewstate form
        viewstate_next_batch = np.zeros(shape=(batch_size,461))
        
        for ii in range(batch_size):
            viewstate_cur_batch[ii] = np.reshape(qf.agent_view(batch[ii].cur_gs),(1,461))
            viewstate_next_batch[ii] = np.reshape(qf.agent_view(batch[ii].next_gs),(1,461))
    ...
    for ii in range(10):
        ...
        for iii in range(500):
            ...
            viewstate = np.reshape(qf.agent_view(qf.gamestate(game)),(1,461))
            #move forward in Q
